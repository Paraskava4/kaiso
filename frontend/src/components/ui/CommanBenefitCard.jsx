...
export const CommanBenefitCard = ({ iconSrc, iconAlt, title, hoverDetails }) => {
    ...
    return (
        <article
            className={`relative snap-center flex-shrink-0 ${isMobileView ? "w-[240px] max-w-[300px]" : "w-full"
        ...
        >
            ...
            <div className="flex justify-center">
                <div
                    className={`flex justify-center items-center bg-[#F4F5F7] h-[76px] w-[76px] rounded-full mb-4 transform transition-all duration-300 ${isHovered ? "opacity-0 scale-75 -translate-y-5" : "opacity-100 scale-100 translate-y-0"
                        }`}
                >
            ...
            {/* Title */}
            <div
                className={`font-semibold text-[14px] mt-10 text-zinc-900 text-center transition-all duration-300 break-words max-w-full px-1 ${isHovered ? "-translate-y-24" : "translate-y-0"
                    }`}
            >
                {title}
            </div>
            ...
            {/* Hover details */}
            <p
                className={`absolute break-words text-xs text-zinc-600 leading-relaxed transition-all duration-300 px-1 text-center top-[120px] right-2 ${isHovered ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10 pointer-events-none"
                    }`}
            >
                {hoverDetails}
            </p>
            ...
            <div className="px-[7.9%]">
                ...
                <div className="relative flex items-center gap-5">
                    <button
                        ...
                        className={`flex bg-white p-2 lg:p-3 rounded-full shadow-md hover:bg-gray-100 transition-colors disabled:opacity-50 disabled:cursor-not-allowed ${isMobileView ? "absolute left-0 -ml-4 z-10 bg-opacity-90" : ""
                            }`}
                    ...
                    >
                        {(isMobileView ? data : currentCards).map((item) => (
                            <CommanBenefitCard
                                key={item.id}
                    ...
                    <button
                        onClick={handleNext}
                        className={`flex bg-white p-2 lg:p-3 rounded-full shadow-md hover:bg-gray-100 transition-colors disabled:opacity-50 disabled:cursor-not-allowed ${isMobileView ? "absolute right-0 -mr-4 z-10 bg-opacity-90" : ""
                            }`}
