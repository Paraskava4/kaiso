...
export const FeatureCard = ({ title = "Default Title", description = "Default description" }) => {
    return (
        <article className="min-w-[185px] px-4 py-6 bg-white border border-gray-600 rounded-3xl snap-start  transition-all duration-300 ease-in-out
...
export const CommanFeatureCardContainer = ({ data, title }) => {
    ...
    const handleNext = () => {
        setCurrentIndex((prev) =>
            Math.min(data.length - cardsPerView, prev + 1)
        );
    };
        ...
        <section className="w-full max-w-full mx-auto px-4 md:px-6 lg:px-8 py-12 bg-[#F4F5F7]">
            <div className="px-[7.9%]">
                ...
                <div className="flex items-center gap-3 sm:gap-5">
                    ...
                    >
                        {(isMobileView ? data : currentCards).map((feature) => (
