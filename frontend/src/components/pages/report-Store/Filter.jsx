    ...
    const renderIndustry = (title, subCategories = [], catId) => {
        ...
        return (
            <div key={title} className="w-full border-b border-zinc-300 last:border-b-0">
                <div
                    className={`flex justify-between items-center py-3 sm:py-2.5 text-sm sm:text-base font-medium text-zinc-700 cursor-pointer hover:bg-zinc-50 rounded-md transition-colors ${selectedCategory === catId ? "text-[#d5003c]" : ""
                        }`}
                    onClick={() => {
                        if (hasChildren) {
                    ...
                    <div className="pl-4 pr-2 pb-2 space-y-1">
                        {filteredSubs?.map((item) => (
                            <div
                                ...
                                className={`py-2 px-2 cursor-pointer hover:bg-zinc-50 rounded-md transition-colors text-sm ${selectedSubCategory === item._id ? "font-medium text-[#d5003c]" : "text-zinc-700"
                                    }`}
                            >
                                {item.name}
                            </div>
...
export const UrlFilter = ({ urlData, reportData, onFilterChange, searchParams }) => {
    ...
    const onChangeSearchText = (text) => {
        // setSearchText(text);
        if (text) {
            redirect(`report-store?search=${encodeURIComponent(text.trim())}`);
            setSearchText(text);
        } else {
            redirect(`report-store`);
            setSearchText(text);
                        ...
                        categoryAndSubCategory.map((category) => {
                            ...
                            return (
                                <div key={category._id} className={`w-full border-b border-zinc-300 last:border-b-0`}>
                                    <div
                                        className={`flex justify-between items-center py-2 sm:py-2 text-[14px] font-medium cursor-pointer rounded-md transition-colors ${isActiveCategory ? "text-[#d5003c]" : "text-zinc-700"
                                            }`}
                                        onClick={() => handleOnClickCategory(category)}
                                    >
                                        <span className="flex-1 pr-2 hover:text-[#d5003c] font-medium text-[13px]">{category.name}</span>
                                        {hasChildren && (
                                            <Button
                                                onClick={(e) => {
                                                    e.stopPropagation();
                                        ...
                                        <div className="pl-4 pr-2 pb-2 space-y-1">
                                            {category.subCategories?.map((sub) => {
                                                ...
                                                return (
                                                    <div
                                                        key={sub._id}
                                                        className={`py-2 px-2 cursor-pointer rounded-md transition-colors hover:text-[#d5003c] text-[13px] ${isActiveSub ? "text-[#d5003c]" : "text-zinc-700"
                                                            }`}
