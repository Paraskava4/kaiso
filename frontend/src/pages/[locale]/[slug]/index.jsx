...
const Index = ({ seoData, pageData, error, timestamp }) => {
    ...
    const renderContent = useMemo(() => {
        ...
        // Use pageData if available, otherwise show loading
        const displayData = pageData;

        // 1. Success with main query
        if (displayData && pageType === "report-store") {
            return (
                <Suspense fallback={<div className="animate-pulse bg-gray-200 h-96 rounded-lg"></div>}>
                    <ReportDetailsPage reportData={displayData} />
                </Suspense>
            );
        }

        if (displayData && pageType === "blog") {
            return (
                <Suspense fallback={<div className="animate-pulse bg-gray-200 h-96 rounded-lg"></div>}>
                    <BlogDetail slugData={displayData} />
                </Suspense>
            );
        }

        // 2. Fallback to category data
        if (displayData && Array.isArray(displayData) && pageType === "report-store") {
            return (
                <Suspense fallback={<div className="animate-pulse bg-gray-200 h-96 rounded-lg"></div>}>
                    <ReportStorePage
        ...
        if (displayData && Array.isArray(displayData) && pageType === "blog") {
            return (
                <Suspense fallback={<div className="animate-pulse bg-gray-200 h-96 rounded-lg"></div>}>
                    <FeaturedBlogs blogsData={displayData} />
                </Suspense>
            );
        }
    ...
    }, [pageType, pageData, error, pageSlug]);
