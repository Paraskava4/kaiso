...
// Mapping between frontend column IDs and backend field names for Other
const columnToFieldMapping = {
    inquiryNo: "inquiryNo",
    clientName: "firstName",
    pageName: "pageName", // This might need to be derived from type or other field
...
// Column definitions for Other Inquiries page
const initialColumns = [
    { id: "inquiryNo", label: "Inquiry No", sortable: true },
    { id: "clientName", label: "Client Name", sortable: true },
    { id: "pageName", label: "Page Name", sortable: true },
...
const Other = () => {
    ...
    const mapInquiryToRow = (item, index) => ({
        inquiryNo: item.inquiryNo ? item.inquiryNo : index + 1,
        clientName: `${item.firstName || ""} ${item.lastName || ""}`.trim(),
        pageName: item.pageName || "-", // Use type as page name since it's "Other"
    ...
    function handleExport() {
        ...
        exportToExcel(selectedRows, columns, "other_inquiries.xlsx");
    }
    ...
    const handleStar = async (id) => {
        try {
            ...
            const inquiry = rows.find((row) => row.inquiryNo === id);
            ...
            if (toggleInquiryLike.fulfilled.match(result)) {
                ...
                setRows((prev) => prev.map((row) => (row.inquiryNo === id ? { ...row, starred: !row.starred } : row)));
            } else {
                // Show error toast
    ...
    // Handle star sorting - sort by starred field (client-side)
    ...
    const formatInquiryForInfoTable = (inquiry) => {
        ...
        // Create an array of objects for InfoTable - Other inquiry specific fields
        return [
            {
                label: "Inquiry ID",
            ...
            { label: "Page Name", value: inquiry.pageName || rawData.type || "Other" },
            { label: "Company Name", value: rawData.companyName || inquiry.companyName || "-" },
    ...
    return (
        <div className="flex flex-col gap-4 w-full">
            {/* Page heading */}
            <PageHeader title="Other Inquiries" rightContent={headerRight} />
