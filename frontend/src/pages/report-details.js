...
function ReportDetailsPage(props) {
    ...
    useEffect(() => {
        ...
        if (reportData) {
            setReport(transformReport(reportData)); // transform if necessary
            setLoading(false);
            return;
        }
        if (!reportId) return;
        ...
        return (
            <div className="min-h-screen bg-white flex items-center justify-center">
                <div className="text-center">
                    ...
                    <button onClick={() => router.push("/report-store")} className="bg-sky-900 text-white px-6 py-2 rounded hover:bg-sky-800 transition-colors">
                        Browse Reports
                    </button>
                </div>
            </div>
