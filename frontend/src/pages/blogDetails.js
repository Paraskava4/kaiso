...
export default function BlogDetail(props) {
    ...
    useEffect(() => {
        if (!slugData && router.isReady && id) {
            setDataId(id);
        }
    }, [router.isReady, id, slugData]);
        ...
        return (
            <div className="flex items-center  justify-center h-screen bg-[#F9F8F4]">
                <div className="text-center">
                    ...
                    <button onClick={() => router.push("/blog")} className="bg-gray-900 text-white px-6 py-2 rounded hover:bg-gray-800 transition-colors">
                        Back to Blogs
                    </button>
                </div>
            </div>
    ...
    return (
        <>
            <section className="p-6 pb-35 mx-auto flex flex-col  bg-[#F9F8F4] md:flex-row gap-6 h-[100%]">
                <div className="mx-auto flex flex-col bg-[#F9F8F4] md:flex-row gap-6 mt-9 w-full" style={{ maxWidth: windowWidth > 1200 ? "83.5%" : "100%" }}>
                    <div className="bg-white w-full overflow-hidden">
                        <div className="w-full px-4 md:px-8">
                            <p className="text-gray-500 mb-4 mt-6 text-sm">
                    ...
                    <div className="md:w-78 w-full">
                        <div
                            ...
                            style={{ position: "sticky", top: "170px", width: "inherit", maxWidth: "inherit", zIndex: 0 }}
                        >
                            ...
                            <ul className="text-gray-600">
                                {similarReportsData?.slice(0, 6).map((report, index) => (
                                    <li
                                        key={index}
                                        onClick={() => router.push(`/report-details/?reportId=${report?._id}`)}
                            ...
                            <ul className="text-gray-600">
                                {similarNewsArticlesData?.map((news, index) => (
                                    <li
                                        key={index}
                                        onClick={() => setDataId(news?._id)}
                                        className="p-2 border-b border-gray-200 hover:text-rose-600 text-sm cursor-pointer"
                                    >
                                        {/* {news?.blogTitle} */}
