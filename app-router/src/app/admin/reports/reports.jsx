                ...
                Cell: ({ row }) => {
                    ...
                    return (
                        <Box sx={{ display: "flex", gap: 0.5 }}>
                            ...
                            <IconButton size="small" color="error" onClick={() => handleDelete(row?.original)} disabled={isButtonDisabled("delete")}>
        ...
        const csvRows = [
            ...
            ...data.map((row) => {
                ...
                const fullUrl = row?.url 
                    ? `${window.location.protocol}//${window.location.host}/report-store/${row.url}`
                    : "";
                ...
                return [
                    ...
                    escapeCsvField(stripHtmlTags(row?.reportOverview)),
                    escapeCsvField(stripHtmlTags(row?.tableOfContent)),
                    escapeCsvField(row?.availableIn),
                    escapeCsvField(row?.internetHandlingCharge),
                    escapeCsvField(row?.seoTitle),
                    escapeCsvField(row?.metaDescription),
                    escapeCsvField(row?.keywords),
                    escapeCsvField(fullUrl),
                ].join(",");
            }),
        ];
