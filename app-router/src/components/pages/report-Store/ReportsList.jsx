...
const ReportsHeader = ({ resultsCount, onSortChange, searchParams }) => (
    <header className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 pb-3 w-full border-b-[0.5px] border-[rgba(67,70,75,0.30)]">
        ...
        <div className="text-[18px] font-[600]">{searchParams?.length > 0 ? `Search result for "${searchParams}"` : "Report Store"}</div>
...
const ActionButtons = ({ id, report }) => {
    ...
    const handleRequestSample = useCallback(
        (e) => {
            e.stopPropagation();
            redirect(`report-store/${report?.url}#sample`);
        },
        [report?.url, router]
    );
...
const ReportsList = ({ selectedCategory, selectedSubCategory, urlName }) => {
    ...
    const { data: subCategoryData, isLoading: isSubCategoryLoading } = useGetDataBySubCategoryQuery(
        { id: selectedSubCategory, pageNo, pageSize },
        {
    ...
    const { data: categoryData, isLoading: isCategoryLoading } = useGetDataByCategoryQuery(
        { id: selectedCategory, pageNo, pageSize },
        {
    ...
    return (
        <main className="w-full" role="main" aria-label="Reports list">
            ...
            <div className="flex justify-center items-center gap-2 py-4 bg-gray-100">
                ...
                <button onClick={handlePrevious} className="px-4 py-1 bg-gray-300 rounded" disabled={pageNo === 1}>
                    Previous
                </button>
                <span>{pageNo}</span>
                <span>{totalPages > 1 ? `of ${totalPages}` : ""}</span>
                <button onClick={handleNext} className="px-4 py-1 bg-green-600 text-white rounded" disabled={pageNo === totalPages}>
                    Next
                </button>
            </div>
        </main>
                ...
                <div className="flex items-center gap-2">
                    <button
                        ...
                        className={`px-3 py-1 rounded border text-sm transition ${
                            pageNo === 1 ? "bg-gray-200 text-gray-400 cursor-not-allowed" : "bg-white text-gray-700 border-gray-300 hover:bg-gray-100"
                        }`}
                    >
                        Previous
                    </button>
                    ...
                    <div className="flex items-center gap-1 text-gray-700">
                        ...
                        {totalPages > 1 && <span className="text-gray-500 text-xs">of {totalPages}</span>}
                    </div>

                    <button
                        onClick={handleNext}
                        disabled={pageNo === totalPages}
                        className={`px-6 py-1 rounded border text-sm transition ${
                            pageNo === totalPages ? "bg-gray-200 text-gray-400 cursor-not-allowed" : "bg-[#054A6E] text-white border-[#054A6E] "
                        }`}
                    >
                        Next
                    </button>
